...
// ========== My define ==========
QChar::Category QChar::category() const noexcept
{
    return QChar::category(ucs);
}

QChar::Direction QChar::direction() const noexcept
{
    return QChar::direction(ucs);
}

QChar::JoiningType QChar::joiningType() const noexcept
{
    return QChar::joiningType(ucs);
}

unsigned char QChar::combiningClass() const noexcept
{
    return QChar::combiningClass(ucs);
}

QChar QChar::mirroredChar() const noexcept
{
    return QChar(QChar::mirroredChar(ucs));
}

bool QChar::hasMirrored() const noexcept
{
    return QChar::hasMirrored(ucs);
}

QChar::Decomposition QChar::decompositionTag() const noexcept
{
    return QChar::decompositionTag(ucs);
}

int QChar::digitValue() const noexcept
{
    return QChar::digitValue(ucs);
}

QChar QChar::toLower() const noexcept
{
    return QChar(QChar::toLower(ucs));
}

QChar QChar::toUpper() const noexcept
{
    return QChar(QChar::toUpper(ucs));
}

QChar QChar::toTitleCase() const noexcept
{
    return QChar(QChar::toTitleCase(ucs));
}

QChar QChar::toCaseFolded() const noexcept
{
    return QChar(QChar::toCaseFolded(ucs));
}

QChar::Script QChar::script() const noexcept
{
    return QChar::script(ucs);
}

QChar::UnicodeVersion QChar::unicodeVersion() const noexcept
{
    return QChar::unicodeVersion(ucs);
}

bool QChar::isPrint() const noexcept
{
    return QChar::isPrint(ucs);
}

bool QChar::isSpace() const noexcept
{
    return QChar::isSpace(ucs);
}

bool QChar::isMark() const noexcept
{
    return QChar::isMark(ucs);
}

bool QChar::isPunct() const noexcept
{
    return QChar::isPunct(ucs);
}

bool QChar::isSymbol() const noexcept
{
    return QChar::isSymbol(ucs);
}

bool QChar::isLetter() const noexcept
{
    return QChar::isLetter(ucs);
}

bool QChar::isNumber() const noexcept
{
    return QChar::isNumber(ucs);
}

bool QChar::isLetterOrNumber() const noexcept
{
    return QChar::isLetterOrNumber(ucs);
}

bool QChar::isDigit() const noexcept
{
    return QChar::isDigit(ucs);
}

bool QChar::isLower() const noexcept
{
    return QChar::isLower(ucs);
}

bool QChar::isUpper() const noexcept
{
    return QChar::isUpper(ucs);
}

bool QChar::isTitleCase() const noexcept
{
    return QChar::isTitleCase(ucs);
}

bool QChar::isNonCharacter() const noexcept
{
    return QChar::isNonCharacter(ucs);
}

bool QChar::isHighSurrogate() const noexcept
{
    return QChar::isHighSurrogate(ucs);
}

bool QChar::isLowSurrogate() const noexcept
{
    return QChar::isLowSurrogate(ucs);
}

bool QChar::isSurrogate() const noexcept
{
...
{
    char16_t chRet = foldCase(ch.unicode());
    return QChar(chRet);
}
// ========== My define ==========
